...
function AdminSubscriptions() {
  ...
  const fetchSubscriptions = async () => {
    try {
      ...
      if (filter === 'pending') {
        ...
        const res = await axios.get(`${apiUrl}/api/subscriptions/admin/all?status=${filter}`);
        if (res.data.success) {
  ...
  const handleApprove = async (subscriptionId) => {
    try {
      ...
      const res = await axios.post(
        `${apiUrl}/api/subscriptions/admin/approve/${subscriptionId}`,
        { status: 'active' }
      );

      if (res.data.success) {
        // Send email notification via Mailpit/SendPulse
        await axios.post(`${apiUrl}/api/send-subscription-email`, {
          email: selectedSubscription.email,
          userName: selectedSubscription.fullname,
          type: 'approved',
          planName: selectedSubscription.name,
          endDate: res.data.end_date
        });

        alert('Subscription approved! Email confirmation sent to user.');
      ...
      alert(err.response?.data?.message || 'Failed to approve subscription');
    }
  };
  ...
  const handleReject = async (subscriptionId) => {
    ...
    try {
      ...
      const res = await axios.post(
        `${apiUrl}/api/subscriptions/admin/reject/${subscriptionId}`,
        { 
          status: 'rejected',
          reason: rejectReason 
        }
      ...
      if (res.data.success) {
        ...
        alert('Subscription rejected! Email notification sent to user.');
        setSelectedSubscription(null);
        setShowModal(false);
        setRejectReason('');
        fetchSubscriptions();
      ...
      alert(err.response?.data?.message || 'Failed to reject subscription');
    }
  };
              ...
              {selectedSubscription.status === 'pending' && (
                <div className="action-section">
                  ...
                  <div className="button-group">
                    ...
                    >
                      <Check size={18} /> Approve Subscription
                    </button>
                    <button
