...
function PaymentModal({ isOpen, onClose, user, onSubscriptionSuccess }) {
  ...
  const fetchPlans = async () => {
    try {
      ...
      const res = await axios.get(`${apiUrl}/api/subscriptions/plans`);
      if (res.data.success) {
  ...
  const handleSubmitPayment = async (e) => {
    ...
    try {
      ...
      console.log('[v0] Uploading payment screenshot to MinIO...');
      const uploadRes = await axios.post(`${apiUrl}/api/upload-file`, formData, {
      ...
      console.log('[v0] Creating subscription request...');
      const subRes = await axios.post(`${apiUrl}/api/subscriptions/request`, {
        user_id: user.id,
        email: user.email,
      ...
      if (subRes.data.success) {
        console.log('[v0] Subscription request created successfully');
        setSuccess(true);
        setStep(3);
        setTimeout(() => {
        ...
        throw new Error(subRes.data.message || 'Failed to create subscription request');
      }
    } catch (err) {
